...
test("computeVersionSuffix: is deterministic and 3 hex chars", () => {
    const a = computeVersionSuffix("a message here", "2.1.252")
    const b = computeVersionSuffix("a message here", "2.1.252")
    assert.equal(a, b)
...
test("buildBillingHeaderValue: well-formed header", () => {
    ...
    assert.match(
        header,
        /^x-anthropic-billing-header: cc_version=2\.1\.252\.[0-9a-f]{3}; cc_entrypoint=sdk-cli; cch=[0-9a-f]{5};$/,
    )
})
...
test("buildUserAgent: default Claude Code form", () => {
    ...
    assert.equal(buildUserAgent(), "claude-cli/2.1.252 (external, sdk-cli)")
})
